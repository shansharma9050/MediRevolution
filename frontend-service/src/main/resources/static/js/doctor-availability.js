...
async function saveAvailability() {
	...
	if (!payload.availableDate || !startTime || !endTime) {
		showMsg("Date, start time and end time are required");
		return;
	}

	if (timeToMinutes(endTime) <= timeToMinutes(startTime)) {
		showMsg("End time must be greater than start time");
		return;
	}
	...
	try {
		...
		const panel =
			document.getElementById("availabilityFormPanel");
		...
		console.error("Save doctor availability error:", e);
...
async function loadDoctorAvailability() {
	...
	try {
		...
		console.error("Load doctor availability error:", e);
		showMsg("Doctor service not reachable.");
		doctorAvailability = [];
		renderAvailability();
		updateAvailabilitySummary();
				...
				<td colspan="6">
					<div class="availability-empty-state">
						...
						<p class="text-muted mb-0">
							Add your first date and time window to start accepting appointments.
						</p>
	...
	doctorAvailability.forEach((a, index) => {
		html += `
			<tr style="--row-delay:${Math.min(index * 55, 330)}ms">
				...
				<td>
					<span class="availability-date-pill">
				...
				<td>
					<span class="availability-slot-pill">
...
function showAvailabilityLoadingState() {
	...
	table.innerHTML = `
		<tr>
			<td colspan="6">
				<div class="availability-loading-state">

					<div class="availability-loading-icon">
...
function updateAvailabilitySummary() {
	const list =
		Array.isArray(doctorAvailability)
			? doctorAvailability
			: [];
...
function calculateSlotCount(a) {
	if (!a.startTime || !a.endTime || !a.slotDuration) {
		return 0;
	}
...
function clearForm() {
	...
	const slotDuration =
		document.getElementById("slotDuration");
