...
public interface SaasExpiryActionRepository extends JpaRepository<SaasExpiryAction, Long> {
	...
	@Query("SELECT a " +
	        ...
	        "AND (" +
	        "LOWER(a.actionNumber) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
	        "OR LOWER(a.medicineName) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
	        "OR LOWER(a.batchNumber) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
	        "OR LOWER(COALESCE(a.supplierName, '')) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
	        "OR LOWER(COALESCE(a.referenceNumber, '')) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
	        "OR LOWER(COALESCE(a.purchaseReturnNumber, '')) LIKE LOWER(CONCAT('%', :keyword, '%'))" +
	        ") " +
	...
	@Query("SELECT COALESCE(SUM(a.stockValue), 0) " +
	        "FROM SaasExpiryAction a " +
	        "WHERE a.tenantId = :tenantId " +
	...
	@Query("SELECT COALESCE(SUM(a.actionQuantity), 0) " +
	        "FROM SaasExpiryAction a " +
	        "WHERE a.tenantId = :tenantId " +
	        "AND a.actionType = :actionType " +
	        "AND a.actionStatus <> " +
