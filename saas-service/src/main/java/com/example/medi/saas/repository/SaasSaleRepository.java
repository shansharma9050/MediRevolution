...
public interface SaasSaleRepository extends JpaRepository<SaasSale, Long> {
	...
	@Query("SELECT s " +
			...
			"AND (" +
			"LOWER(s.saleNumber) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
			"OR LOWER(s.customerName) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
			"OR LOWER(COALESCE(s.customerCode, '')) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
			"OR LOWER(COALESCE(s.customerGstin, '')) LIKE LOWER(CONCAT('%', :keyword, '%'))" +
			") " +
	...
	@Query("SELECT COALESCE(SUM(s.paidAmount), 0) " +
			"FROM SaasSale s " +
			"WHERE s.tenantId = :tenantId " +
			"AND s.saleStatus <> " +
			"com.example.medi.saas.enums.SaasSaleStatus.CANCELLED")
