...
public class SaasDiagnosticService {
	...
	public SaasDiagnosticService(SaasDiagnosticTestRepository testRepository,
			...
			SaasPatientRepository patientRepository, SaasDoctorProfileRepository doctorRepository,
			SaasBillingService billingService, TenantAccessService tenantAccessService,
	...
	public SaasDiagnosticTestResponse createTest(SaasDiagnosticTestRequest request) {
		...
		permissionService.requirePermission(request.getTenantId(), getDiagnosticModule(request.getDiagnosticType()),
	...
	public List<SaasDiagnosticTestResponse> getTests(Long tenantId, String type) {
		...
		permissionService.requirePermission(tenantId, getDiagnosticModule(type), SaasPermissionAction.VIEW);
	...
	public SaasDiagnosticOrderResponse getOrder(Long tenantId, Long orderId) {
		...
		SaasDiagnosticOrder order = orderRepository.findByIdAndTenantIdAndActiveTrue(orderId, tenantId)
				.orElseThrow(() -> new RuntimeException("Diagnostic order not found"));

		permissionService.requirePermission(tenantId, getDiagnosticModule(order.getDiagnosticType()),
	...
	public List<SaasDiagnosticOrderResponse> getPatientOrders(Long tenantId, Long patientId) {
		...
		return orderRepository.findByTenantIdAndPatientIdAndActiveTrueOrderByOrderDateTimeDesc(tenantId, patientId)
				.stream().map(this::toOrderResponse).toList();
	}
	...
	public SaasDiagnosticOrderResponse updateStatus(Long tenantId, Long orderId, String status) {
		...
		SaasDiagnosticOrderStatus newStatus = SaasDiagnosticOrderStatus.valueOf(status.toUpperCase());
	...
	public SaasDiagnosticOrderResponse updateResult(Long orderId, SaasDiagnosticResultRequest request) {
		...
		SaasDiagnosticOrder order = orderRepository.findByIdAndTenantIdAndActiveTrue(orderId, request.getTenantId())
				.orElseThrow(() -> new RuntimeException("Diagnostic order not found"));

		permissionService.requirePermission(request.getTenantId(), getDiagnosticModule(order.getDiagnosticType()),
				SaasPermissionAction.UPDATE);
		...
		notificationService.createSystemNotification(saved.getTenantId(),
				saved.getDiagnosticType() == SaasDiagnosticType.LAB ? SaasNotificationType.LAB_REPORT
						: SaasNotificationType.RADIOLOGY_REPORT,
				SaasNotificationPriority.HIGH, saved.getDiagnosticType().name() + " report ready",
				"Report is ready for order " + saved.getOrderNumber(), saved.getId(), "DIAGNOSTIC_ORDER",
	...
	private SaasDiagnosticTestResponse toTestResponse(SaasDiagnosticTest test) {
		return new SaasDiagnosticTestResponse(test.getId(), test.getTenantId(), test.getDiagnosticType().name(),
				test.getTestName(), test.getTestCode(), test.getCategory(), test.getDescription(), test.getPrice(),
				test.getActive());
	}
	...
	private SaasDiagnosticOrderResponse toOrderResponse(SaasDiagnosticOrder order) {
		...
		if (order.getDoctorProfileId() != null) {
			doctor = doctorRepository.findByIdAndTenantIdAndActiveTrue(order.getDoctorProfileId(), order.getTenantId())
					.orElse(null);
		}
		...
		List<SaasDiagnosticOrderItemResponse> items = orderItemRepository
				...
				.map(item -> new SaasDiagnosticOrderItemResponse(item.getId(), item.getTestId(), item.getTestName(),
						item.getTestCode(), item.getPrice()))
				.toList();

		return new SaasDiagnosticOrderResponse(order.getId(), order.getTenantId(), order.getOrderNumber(),
				order.getDiagnosticType().name(), order.getPatientId(),
				patient == null ? null : patient.getPatientName(), patient == null ? null : patient.getMobile(),
				order.getDoctorProfileId(), doctor == null ? null : doctor.getDoctorName(),
				doctor == null ? null : doctor.getDepartment(), order.getPrescriptionId(), order.getAppointmentId(),
				order.getInvoiceId(), order.getSubtotal(), order.getDiscountAmount(), order.getTaxAmount(),
				order.getTotalAmount(), order.getStatus().name(), order.getClinicalNotes(), order.getResultSummary(),
				order.getResultDetails(), order.getReportFileUrl(), order.getSampleCollectedAt(),
				order.getReportReadyAt(), order.getOrderDateTime(), items);
	}
	...
	private TenantModule getDiagnosticModule(String type) {
		...
		return type.equalsIgnoreCase("LAB") ? TenantModule.LAB : TenantModule.RADIOLOGY;
	}
