...
public interface SaasPurchaseReturnRepository extends JpaRepository<SaasPurchaseReturn, Long> {
	...
	@Query("SELECT r " +
			...
			"OR LOWER(r.purchaseNumber) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
			"OR LOWER(r.supplierName) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
			"OR LOWER(COALESCE(r.supplierInvoiceNumber, '')) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
			"OR LOWER(COALESCE(r.debitNoteNumber, '')) LIKE LOWER(CONCAT('%', :keyword, '%'))" +
			") " +
