...
public class SaasPrescriptionPdfService {
	...
	public byte[] generatePrescriptionPdf(Long tenantId, Long prescriptionId) {
		...
		permissionService.requirePermission(tenantId, TenantModule.PRESCRIPTIONS, SaasPermissionAction.PRINT);

		SaasPrescriptionResponse prescription = prescriptionService.getPrescription(tenantId, prescriptionId);

		return generatePdf(tenantId, prescription);
	}
	 ...
	 * ================================================================
	 *
	 * Ownership is checked by getMyPrescription().
	 */

	public byte[] generatePatientPrescriptionPdf(Long tenantId, Long prescriptionId) {

		SaasPrescriptionResponse prescription = prescriptionService.getMyPrescription(tenantId, prescriptionId);
	...
	private byte[] generatePdf(Long tenantId, SaasPrescriptionResponse prescription) {
		...
		try {
			...
			addSection(document, "Diagnosis", prescription.getDiagnosis(), headingFont, normalFont);

			addSection(document, "Clinical Notes", prescription.getClinicalNotes(), headingFont, normalFont);
			...
			addSection(document, "Advice", prescription.getAdvice(), headingFont, normalFont);

			addSection(document, "Lab Tests", prescription.getLabTests(), headingFont, normalFont);
			...
			brandingService.addFooter(document, tenantId,
					brandingService.getBranding(tenantId).getPrescriptionFooter());
			...
			throw new RuntimeException("Unable to generate prescription PDF", exception);
		}
	}

	private void addSection(Document document, String title, String value, Font headingFont, Font normalFont)
			throws DocumentException {

		document.add(new Paragraph(title, headingFont));

		document.add(new Paragraph(safe(value), normalFont));
